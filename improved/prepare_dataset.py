...
def load_reliable_sources(news_data_csv: Path) -> dict[str, ReliableSource]:
    ...
    for u in df["link"].astype(str).tolist():
        ...
        if not d:
            continue
        # simple policy: if it appears in news_data.csv, treat as reliable
...
def build_from_dataset_csv(dataset_csv: Path) -> pd.DataFrame:
    ...
    # expects: idd,title,text,label (seen in your repo)
...
def add_reliability_features(df: pd.DataFrame, reliable_domains: dict[str, ReliableSource]) -> pd.DataFrame:
    # If the base dataset has no URL/domain, mark as unknown.
    # These features still help because injected reliable headlines have known domains.
    df = df.copy()
    df["source_domain"] = ""
...
def main():
    ...
    ap.add_argument("--reliable_real_multiplier", type=int, default=5, help="how many times to upweight reliable real headlines")
    args = ap.parse_args()
    ...
    base = build_from_dataset_csv(dataset_csv)
    base = add_reliability_features(base, reliable_domains)
    ...
    # take one row per group to split group ids, then filter
    ...
    val.to_csv(val_path, index=False, quoting=csv.QUOTE_MINIMAL)
